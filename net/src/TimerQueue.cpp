...
void TimerQueue::insertTimerEvent(TimeStamp timeStamp, std::size_t interval, TIMER_CALLBACK&& callback, std::size_t count){

    if(count == 0) {
        LOG_WARN << TimeStamp::now().whenCreate_str() << " insert timerEvent time=[" << timeStamp.whenCreate_str() 
                 << "], interval=[" << interval << "], but count=[0] so refuse to insert!"; 
        return;
    }
